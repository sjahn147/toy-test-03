...
assert.equal(sim.agents.length, 9, 'WP8-C must expose exactly nine cumulative heroes');
assert.equal(new Set(sim.agents.map(agent => agent.heroId)).size, 9, 'hero identities must remain unique');
assert.equal(placed.length, 9, 'every hero must receive an occupancy placement');

system.initialize(sim);
assert.equal(sim.agents.length, 9, 'reinitialization must not duplicate unique heroes');
