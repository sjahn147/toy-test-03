...
// Hero death cleanup must invalidate every field owned by that agent.
{
  const cleanup = new HeroEnvironmentSystem();
  const room = { id: 'D20', tags: [] };
  const owner = { id: 'cleanup-owner', heroId: 'hero.jijik', faction: 'dungeon', ecologyFaction: 'goblin-clan', roomId: room.id, alive: true };
  const cleanupSim = { rooms: [room], agents: [owner], emitEffect() {} };
  cleanup.createField({ kind: 'generic', ownerHeroId: owner.heroId, ownerAgentId: owner.id, factionId: owner.ecologyFaction, roomId: room.id, duration: 20 }, cleanupSim);
  assert.equal(cleanup.clearOwner(owner.id, cleanupSim), 1);
  cleanup.update(0, cleanupSim);
  assert.equal(cleanup.fields.size, 0);
}
