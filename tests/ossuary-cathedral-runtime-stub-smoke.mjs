...
// a plain ReferenceError inside a builder (e.g. a helper used but not imported)
// fails this test instead of slipping through undetected.

const sourceRoot = new URL('../src/engine/', import.meta.url);
const stubPath = new URL('./support/ThreeSceneStub.js', import.meta.url);
const tempRoot = await mkdtemp(join(tmpdir(), 'ossuary-cathedral-runtime-'));
...
const packModule = await import(pathToFileURL(join(engineRoot, 'OssuaryCathedralAssetPack.js')).href);
const recipeModule = await import(pathToFileURL(join(engineRoot, 'OssuaryCathedralLandmarkRecipes.js')).href);
const { OssuaryCathedralAssetPack } = packModule;
const { listOssuaryCathedralLandmarkRecipes } = recipeModule;
const pack = new OssuaryCathedralAssetPack();
const recipes = listOssuaryCathedralLandmarkRecipes();
const results = [];

assert.equal(recipes.length, 5, 'runtime smoke expected five ossuary recipes');
...
assert.equal(pack.canCreate('ossuary.missing'), false, 'unknown bundle must not be claimed');
assert.equal(pack.create('ossuary.missing'), null, 'unknown bundle must fail softly');

console.log('ossuary cathedral runtime stub smoke passed');
