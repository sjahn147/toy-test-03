...
// Floor spacing was raised from 2.85 to 5.4 world units by the height-stability fix.
assert.deepEqual({ x: renderer.deployables.get('charge').position.x, y: renderer.deployables.get('charge').position.y, z: renderer.deployables.get('charge').position.z }, { x: 11, y: 5.4, z: -2 });
assert.ok(renderer.projectiles.get('shell').position.y > 5.4);
assert.ok(Math.abs(renderer.fields.get('drain').position.y - 5.435) < 1e-9);
