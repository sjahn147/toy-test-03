...
const [outpostFactorySource, registrySource, overlaySource, rendererSource, observerSource] = await Promise.all([
...
assert.equal(isForwardOutpostProp({ type: 'territory_banner', label: 'Goblin Forward Outpost' }), true);
...
assert.match(observerSource, /class StrategyObserverScreen/);
assert.match(observerSource, /StrategyDungeonRenderer/);
assert.match(observerSource, /StrategyAssetRegistry/);
assert.match(observerSource, /extends Phase8ObserveScreen/);
