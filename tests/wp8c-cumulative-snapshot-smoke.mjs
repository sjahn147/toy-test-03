...
const rooms = [...new Set(listHeroDefinitions().map(hero => hero.initialRoomId))].map((id, index) => ({ id, x: index * 18, z: 0, w: 16, d: 14, tags: id === 'C14' ? ['flooded'] : [] }));
const sim = {
  time: 0,
  rooms,
  agents: [],
  ...
  occupancy: {
    placeAgent(agent, roomId) {
      ...
      agent.roomId = roomId;
      agent.roomCell = { x: room?.x ?? 0, z: room?.z ?? 0, footprint: [] };
      return agent.roomCell;
    },
    release() {}, cancelReservation() {}
  },
  ...
  ecosystem: { corpses: [] },
  heroResourceLedger: {
    'goblin-clan': { powder: 10, scrap: 10 },
    'copper-tail-clutch': { scrap: 10 },
    'red-tusk-tribe': { meat: 14 }
  },
  emitEffect(type, payload) { this.effects.push({ type, ...payload }); },
  event() {},
  applyCombatDamage(_source, target, amount) { target.hp = Math.max(0, target.hp - amount); }
};
...
Object.assign(sim, {
  ...
  heroLeadershipSystem: leadership,
  heroFormSystem: forms,
...
assert.equal(payload.heroForms.forms.length, 5);
...
assert.equal(restored.heroForms.forms.filter(form => form.ownerHeroId === 'hero.glop').length, 3);
assert.equal(restored.heroForms.forms.filter(form => form.ownerHeroId === 'hero.isara').length, 2);
...
console.log('WP8-C cumulative forms, physics, deployables, and environment snapshot passed');
