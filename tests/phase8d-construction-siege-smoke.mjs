...
try {
  ...
  const job = sim.constructionSiegeSystem.startConstruction(settlement, 'gatehouse', sim, { force: true });
  ...
  sim.constructionSiegeSystem.advanceJobs(0.1, sim);
  ...
  while (sim.props.includes(prop)) {
    ...
    sim.constructionSiegeSystem.attackStructure(attacker, prop, sim);
  }
  ...
  sim.constructionSiegeSystem.updateSupplyStatus(sim);
