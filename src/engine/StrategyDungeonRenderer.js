...
export class StrategyDungeonRenderer extends DungeonRendererPhase8 {
  constructor(three, scenario, assets) {
    ...
    this.worldStatusOverlay = new WorldStatusOverlayRenderer({
      group: this.group,
  ...
  renderState(snapshot) {
    ...
    const filtered = {
      ...snapshot,
      props: (snapshot.props ?? []).filter(prop => !isForwardOutpost(prop))
    };
  ...
  renderForwardOutposts(outposts, rooms, time) {
    ...
    for (const outpost of outposts) {
      const signature = JSON.stringify([
        outpost.outpostProfile,
        outpost.ecologyFaction,
        Math.floor((outpost.integrity ?? 0) / 8),
        Math.floor((outpost.buildProgress ?? 1) * 10)
      ]);
  ...
  destroy() {
    ...
    for (const mesh of this.forwardOutpostMeshes.values()) {
      this.group.remove(mesh);
      disposeTree(mesh);
    }
