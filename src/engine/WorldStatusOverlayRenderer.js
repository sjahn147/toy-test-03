...
export class WorldStatusOverlayRenderer {
  ...
  renderControlFields(states, rooms, time) {
    ...
    for (const state of visible) {
      ...
      if (!room) continue;
      const signature = `${state.owner}:${Boolean(state.contested)}`;
      let entry = this.controlFields.get(state.roomId);
      if (!entry || entry.signature !== signature) {
        if (entry) this.removeEntry(entry);
        const mesh = createControlField(state);
  ...
  renderActivityBeacons(agents, rooms, time) {
    const active = agents.filter(agent => {
      ...
      return agent.alive && !agent.hidden && !agent.departed && activity && ACTIVE_ACTIVITY_SOURCES.has(activity.source);
    });
    ...
    for (const agent of active) {
      ...
      if (!entry || entry.signature !== signature) {
        if (entry) this.removeEntry(entry);
        const mesh = createActivityBeacon(activity, agent);
        entry = { mesh, signature };
      ...
      entry.mesh.position.set(
        room.x + (anchor.ox ?? 0),
        this.roomY(room) + 0.86 + Math.sin(time * 3.2 + hash(agent.id) * 4) * 0.06,
        room.z + (anchor.oz ?? 0)
      );
      entry.mesh.rotation.y = time * 0.7;
      const scale = 0.78 + Math.max(0, Math.min(1, progress)) * 0.22;
...
function createControlField(state) {
  ...
  group.add(disc);
  if (state.contested) {
    const challenger = new THREE.Mesh(
      new THREE.RingGeometry(0.55, 0.96, 32, 1, 0, Math.PI),
      new THREE.MeshBasicMaterial({ color: factionColor(state.challenger), transparent: true, opacity: 0.22, depthWrite: false, side: THREE.DoubleSide })
    );
...
function createSupplyRoute(cargo, rooms, roomY) {
  const points = cargo.route
    .map(roomId => rooms.get(roomId))
    .filter(Boolean)
    .map(room => new THREE.Vector3(room.x, roomY(room) + 0.13, room.z));
  ...
  const material = new THREE.LineDashedMaterial({ color, transparent: true, opacity: 0.4, dashSize: 0.36, gapSize: 0.22, depthWrite: false });
...
function createActivityBeacon(activity, agent) {
  const group = new THREE.Group();
  const color = activity.source === 'operations'
    ? 0xe0bd68
    : activity.source === 'strategic-expansion'
      ? 0xd66d55
      : agent.faction === 'party' ? 0x74b9e6 : factionColor(agent.ecologyFaction);
