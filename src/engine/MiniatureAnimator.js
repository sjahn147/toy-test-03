...
export class MiniatureAnimator {
  update(mesh, agent, time, effects = []) {
    ...
    const attack = attackTimeline(agent, time, seed);
    const role = agent.role;
    const skeletal = role === 'skeleton';
    const roleLean = role === 'rogue' ? 0.1 : role === 'fighter' ? 0.035 : role === 'wizard' ? 0.02 : skeletal ? 0.055 : 0.045;
    const stride = moving ? (role === 'rogue' ? 0.62 : role === 'goblin' ? 0.7 : role === 'ogre' ? 0.44 : 0.52) : 0;
    const armSwing = moving ? stride * 0.78 : 0;
    const idleBreath = Math.sin(time * (skeletal ? 2.4 : 1.55) + seed * 5.4);
    ...
    const attackLean = attack.windup * -0.14 + attack.strike * 0.22 + attack.recover * 0.04;

    dampPosition(rig.pelvis, 'y', rig.pelvis.userData.restY + (moving ? liftWave * 0.028 : skeletal ? 0 : idleBreath * 0.008) - attack.strike * 0.015, alpha);
    ...
    dampRotation(rig.pelvis, 'z', moving ? Math.cos(phase) * 0.025 : skeletal ? idleBreath * 0.028 : idleBreath * 0.018, alpha);

    dampRotation(rig.spine, 'x', roleLean + hitPulse * -0.24 + healPulse * -0.05 + attackLean, alpha);
    dampRotation(rig.spine, 'y', (moving ? -strideWave * 0.075 : idleBreath * -0.014) + attack.twist, alpha);
    dampRotation(rig.chest, 'x', hitPulse * -0.28 + (moving ? 0.015 : idleBreath * 0.012) + attackLean * 0.65, alpha);
    dampRotation(rig.chest, 'y', (moving ? -strideWave * 0.09 : idleBreath * 0.02) + attack.twist * 0.78, alpha);
    dampRotation(rig.head, 'x', hitPulse * 0.18 + (role === 'rogue' ? -0.04 : 0) - attack.strike * 0.04, alpha);
    ...
    poseLeg(rig.thighL, rig.shinL, rig.footL, strideWave * stride, moving, attack, -1, alpha);
    poseLeg(rig.thighR, rig.shinR, rig.footR, -strideWave * stride, moving, attack, 1, alpha);
    ...
    const breathScale = 1 + (skeletal ? 0 : idleBreath * 0.004) + healPulse * 0.014;
  ...
  animateCreatureFallback(mesh, agent, time, seed, alpha, effects) {
    ...
    if (agent.role === 'slime') {
      ...
      if (parts.core) {
        parts.core.position.x = 0.1 + Math.sin(phase * 0.7) * 0.07 + attack.strike * 0.06;
        parts.core.position.z = 0.08 + Math.cos(phase * 0.85) * 0.05 + attack.strike * 0.08;
        parts.core.rotation.y += 0.035;
      }
      return;
    }
...
function poseLeg(thigh, shin, foot, wave, moving, attack, side, alpha) {
  ...
  dampRotation(thigh, 'z', moving ? Math.abs(wave) * 0.025 : attack.strike * side * 0.025, alpha);
...
function combatArmPose(role, dominant, attack) {
  ...
  if (role === 'rogue') return { x: attack.windup * 0.58 - attack.strike * 1.08, y: hand * (attack.windup * -0.24 + attack.strike * 0.2), z: hand * attack.strike * -0.18, forearm: 0.42 + attack.windup * 0.42 - attack.strike * 0.24, forearmZ: hand * -attack.strike * 0.12 };
  if (!dominant && role === 'fighter') return { x: -0.2 - attack.strike * 0.12, y: -0.28, z: 0.18, forearm: 0.48, forearmZ: 0 };
...
function attackTimeline(agent, time, seed) {
  if (!agent.combat) return { active: false, windup: 0, strike: 0, recover: 0, twist: 0 };
  const speed = agent.role === 'ogre' ? 0.78 : agent.role === 'rogue' || agent.role === 'goblin' ? 1.28 : 1;
  const cycle = ((time * speed + seed * 0.37) % 1 + 1) % 1;
  ...
  return { active: true, windup, strike, recover, twist: -windup * 0.34 + strike * 0.48 + recover * 0.1 };
}
